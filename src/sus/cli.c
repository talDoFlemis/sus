...
static struct argp_option options[] = {
    ...
    {"path-to-client-process", 'c', "CLIENT_PROCESS", 0,
     "Path to client process."},
    {"patience-in-ms", 't', "NUMBER", 0, "Patience for the project in msec."},
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
  ...
  case 'n':
    if (arg)
      arguments->number_of_clients = strtoull(arg, NULL, 10);
    break;
  case 'p':
    if (arg)
      arguments->max_number_of_processes = (uint8_t)strtoul(arg, NULL, 10);
    break;
  case 'c':
  ...
  case 't':
    if (arg)
      arguments->patience_in_ms = strtoull(arg, NULL, 10);
    break;
  case 's':
    if (arg)
      arguments->seed = strtoull(arg, NULL, 10);
    break;
  case 'l':
  ...
  case 'm':
    if (arg)
      arguments->max_number_of_int_to_read = (uint8_t)strtoul(arg, NULL, 10);
    break;
  case ARGP_KEY_ARG:
    return 0;
  default:
